...
const Matches = () => {
    ...
    useEffect(() => {
        const fetchMatches = async () => {
            try {
                ...
                const [profilesResponse, shortlistRes, interestRes] = await Promise.all([
                    getOppositeGenderProfiles(authData.token).catch(e => {
                        console.error('Error fetching opposite gender profiles:', e);
                ...
                if (profilesResponse.success && profilesResponse.data) {
                    ...
                    if (shortlistRes && shortlistRes.ok) {
                        try {
                            const dl = await shortlistRes.json();
                            ...
                            console.error('Error parsing shortlist response:', e);
                        }
                    }

                    // Process interest data
                    if (interestRes && interestRes.ok) {
                        try {
                            ...
                            console.error('Error parsing interests response:', e);
                        }
                    }
    ...
    // Memoize the filtered profiles calculation to prevent unnecessary recalculations
    ...
    // Memoize pagination variables derived from filteredProfiles
    const { totalProfiles, totalPages, indexOfLastProfile, indexOfFirstProfile, currentProfiles } = useMemo(() => {
        ...
        const current = filteredProfiles.slice(firstIndex, lastIndex);
