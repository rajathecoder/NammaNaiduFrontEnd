    ...
    const filteredProfiles = useMemo(() => {
        if (selectedFilter === 'newly-joined') {
            ...
            return allMatches.filter(match => {
                if (match.createdAt) {
                    // Optimized: Comparing timestamps is faster than full Date objects if many matches exist
                    const createdDate = new Date(match.createdAt).getTime();
                    return createdDate >= fiveDaysAgoTime;
                }
                return false;
            });
        }
