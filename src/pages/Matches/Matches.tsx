...
const Matches = () => {
    ...
    // Memoize the filtered profiles array to prevent O(N) recalculations on every render,
    // especially when only pagination state changes.
    const filteredProfiles = useMemo(() => {
        if (selectedFilter === 'newly-joined') {
            ...
            return allMatches.filter(match => {
                if (match.createdAt) {
                    return new Date(match.createdAt).getTime() >= fiveDaysAgoTime;
                }
                return false;
            });
        }
    ...
    // Memoize the slicing operation to avoid creating a new array reference on renders
    // where filteredProfiles and pagination limits haven't changed.
