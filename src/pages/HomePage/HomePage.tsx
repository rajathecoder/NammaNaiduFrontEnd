        ...
        const fetchOppositeGenderProfiles = async () => {
            ...
            try {
                ...
                if (response.success && response.data) {
                    ...
                    // ⚡ Bolt: Eliminate N+1 API calls for opposite-gender profile photos
                    // The getOppositeGenderProfiles API payload includes the user's photo information
                    // nested in a lowercase personphoto array of objects. We map this directly from
                    // the initial payload to prevent redundant fetching via the GET_PHOTOS endpoint.
                    const photosMap: Record<string, { photo1link?: string }> = {};

                    response.data.forEach((profile: UserProfile & { personphoto?: Array<{ photoplacement: number, photo1link?: string }> }) => {
                        if (profile.personphoto && Array.isArray(profile.personphoto) && profile.personphoto.length > 0) {
                            const photo1 = profile.personphoto.find((p: { photoplacement: number, photo1link?: string }) => p.photoplacement === 1);
