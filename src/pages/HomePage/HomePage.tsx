        ...
        const fetchOppositeGenderProfiles = async () => {
            ...
            try {
                ...
                if (response.success && response.data) {
                    ...
                    // ⚡ Bolt: Eliminate N+1 API calls by mapping pre-fetched photo data
                    // The getOppositeGenderProfiles API already returns a 'personphoto' array in the payload
                    ...
                    response.data.forEach((profile: UserProfile & { personphoto?: Array<{ photoplacement: number, photo1link?: string }> }) => {
                        const photos = profile.personphoto;
                        if (Array.isArray(photos) && photos.length > 0) {
                            const photo1 = photos.find(p => p.photoplacement === 1);
                            if (photo1?.photo1link && photo1.photo1link.trim() !== '' && photo1.photo1link !== 'null') {
