        ...
        const fetchOppositeGenderProfiles = async () => {
            ...
            try {
                ...
                if (response.success && response.data) {
                    ...
                    // ⚡ Bolt: Memoize profile photos from the primary payload.
                    // The backend getOppositeGenderProfiles API payload already includes the user's primary
                    // photo link nested at personPhoto.photo1. This field is mapped directly in frontend state
                    // to prevent redundant N+1 fetching via the separate GET_PHOTOS endpoint.
                    const photosMap: Record<string, { photo1link?: string }> = {};
                    response.data.forEach((profile: UserProfile) => {
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                        const photo1 = (profile.personPhoto as any)?.photo1;
                        if (photo1 && photo1.trim() !== '' && photo1 !== 'null') {
                            photosMap[profile.accountId] = { photo1link: photo1 };
                        }
                    });
