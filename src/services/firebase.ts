...
const ensureFirebaseApp = () => {
  if (!getApps().length) {
    // Validate that all required Firebase config keys are present using static access for Vite
    if (!import.meta.env.VITE_FIREBASE_API_KEY || !import.meta.env.VITE_FIREBASE_AUTH_DOMAIN ||
        !import.meta.env.VITE_FIREBASE_PROJECT_ID || !import.meta.env.VITE_FIREBASE_APP_ID ||
        !import.meta.env.VITE_FIREBASE_MESSAGING_SENDER_ID || !import.meta.env.VITE_FIREBASE_STORAGE_BUCKET) {
      throw new Error('CRITICAL: Missing required Firebase configuration. Secure initialization aborted.');
    }
