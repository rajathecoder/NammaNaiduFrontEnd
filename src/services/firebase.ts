...
const requiredKeys = ['apiKey', 'authDomain', 'projectId', 'appId', 'messagingSenderId', 'storageBucket'] as const;
const missingKeys = requiredKeys.filter(key => !firebaseConfig[key]);

if (missingKeys.length > 0) {
  throw new Error(`Missing required Firebase configuration keys: ${missingKeys.join(', ')}. Please check your .env file.`);
}
